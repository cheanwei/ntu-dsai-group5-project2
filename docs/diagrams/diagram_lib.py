...
def to_drawio(d: Diagram) -> str:
    ...
    for b in sorted(d.boxes, key=lambda b: not b.zone):   # zones first = behind
        ...
        if b.zone:
            style = (f"rounded=1;arcSize=6;whiteSpace=wrap;html=1;dashed=1;dashPattern=8 6;"
            ...
            style = (f"rounded=1;arcSize=8;whiteSpace=wrap;html=1;"
    ...
    for e in d.edges:
        ...
        style = (f"edgeStyle=orthogonalEdgeStyle;rounded=1;html=1;jettySize=auto;"
...
def to_excalidraw(d: Diagram) -> str:
    ...
    for b in sorted(d.boxes, key=lambda b: not b.zone):
        fill, stroke = PALETTE[b.kind]
        rect = _common(b.id, b.x, b.y, b.w, b.h, stroke,
                       "transparent" if b.zone else fill, dashed=b.zone)
        rect["type"] = "rectangle"
        tid = b.id + "_t"
        ...
        tw = round(max(len(l) for l in lines) * fs * _CHAR_W, 1)
        valign = "top" if b.zone else "middle"
        text = _common(tid, b.x + 12,
                       b.y + 8 if b.zone else b.y + (b.h - th) / 2,
                       min(tw, b.w - 24), th,
                       ZONE_LABEL if b.zone else "#1e1e1e", "transparent")
        text.update({
            "type": "text", "text": b.label, "originalText": b.label,
            "fontSize": fs, "fontFamily": 2,
            "textAlign": "left" if b.zone else b.align,
            "verticalAlign": valign, "containerId": b.id,
            "lineHeight": _LINE_H, "autoResize": False, "roundness": None,
        })
        els.append(rect)
        els.append(text)
    ...
    for e in d.edges:
        ...
        ox, oy = e.pts[0]
        arrow = _common(e.id, ox, oy, max(xs) - min(xs), max(ys) - min(ys),
                        "#495057", "transparent")
        arrow.update({
            "type": "arrow",
        ...
        if e.label:
            ...
            lt = _common(e.id + "_l", mid[0] - tw / 2, mid[1] - 24,
                         tw, fs * _LINE_H, "#495057", "transparent")
            lt.update({
                "type": "text", "text": e.label, "originalText": e.label,
                "fontSize": fs, "fontFamily": 2, "textAlign": "center",
                "verticalAlign": "middle", "containerId": None,
                "lineHeight": _LINE_H, "autoResize": True, "roundness": None,
            })
            els.append(lt)
...
def emit(d: Diagram, stem: str, outdir: Path):
    ...
    print(f"  {stem}.drawio  +  {stem}.excalidraw   "
          f"({len(d.boxes)} boxes, {len(d.edges)} edges)")
